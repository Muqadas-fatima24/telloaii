...
export default function Pricing({ data }: { data: PricingSectionProps }) {
  return (
    <section className="w-full bg-white py-10 sm:py-20">
      <Container>       
        {/* Header Section */}
        <div className="md:mb-25 sm:mb-16 mb-8 text-left sm:text-center">
          <p className="font-primary text-green  text-[10px] sm:text-base font-semibold tracking-widest uppercase">
            {data.label}
          </p>
          <h2 className="mt-2 sm:mt-4 font-primary text-black text-[18px] sm:text-[32px] md:text-h2 font-bold leading-[20px]">
              {data.title.before}{" "}
            <span className="font-tertiary sadg text-green font-normal">
              {data.title.highlight} 
            </span>
          </h2>
          <p className="mx-auto mt-2 sm:mt-4sm:text-sm text-[10px] md:text-base max-w-[600px] font-secondary text-black leading-relaxed">
             {data.subtitle}
          </p>
        </div>

        {/* Pricing Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-13 sm:gap-9 items-center">
          {data.plans.map((plan, index) => (
            <div 
              key={index}
              className={`shadow-[0px_4px_30px_0px_#00000026] relative flex flex-col sm:p-6 p-4 md:p-8 rounded-[25px] text-left transition-all duration-300 min-h-[500px] sm:min-h-[600px] ${
                plan.isPopular 
                ? 'bg-back1 text-white shadow-[0_4px_30px_rgba(0,0,0,0.15)] scale-105 z-10 py-12' 
                : 'bg-white border border-[#A7DECA] text-[#1C373F]'
              }`}
            >
              {plan.isPopular && (
                <div className="absolute -top-4 left-1/2 border-[.5px] border-white -translate-x-1/2 bg-green text-white text-sm font-semibold px-2 md:px-5 py-2 rounded-[5px] uppercase tracking-widest">
                  Most Popular
                </div>
              )}

              <h3 className="font-primary text-base md:text-h3 font-semibold">{plan.name}</h3>
              <p className={`mt-3 font-primary font-normal text-sm leading-relaxed ${plan.isPopular ? 'text-white' : 'text-black'}`}>
                {plan.description}
              </p> 

              <div className="mt-5 text-base flex items-baseline gap-1">
                <span className={`text-sm font-primary ${plan.isPopular ? 'text-white' : 'text-green'}`}>From</span>
                <span className={` font-primary md:text-h1 sm:text-h2 text-h3 font-bold ${plan.isPopular ? 'text-white' : 'text-green'}`}>{plan.price}</span>
             ...
             <a href='#'>
              <Button variant="pricing" isPopular={plan.isPopular}  className="font-secondary text-white! hover:text-white/70! !cursor-pointer !font-semibold !rounded-[5px]">
                   {plan.buttonText}
              </Button>
              </a>
            </div>
